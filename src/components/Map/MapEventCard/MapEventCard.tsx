...
export const MapEventCard = ({ event, onClose }: MapEventCardProps) => {
  const images = getEventImages(event)
  const dateTime = formatEventDateTime(event.eventDate, event.startTime)
  
  const guestText = event.minGuests && event.minGuests > 1
  ...
  return (
    <div className={styles.card}>
      ...
      <div className={styles.content}>
        <Text className={styles.title} size="md" weight="semibold">
          {event.title}
        </Text>

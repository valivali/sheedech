      ...
      {STEP_ORDER.map((step, index) => {
        ...
        return (
          ...
          >
            ...
            {step === "personal-info"
              ? "Personal Info"
              : step === "host-preferences"
                ? "Host Preferences"
                : step === "guest-preferences"
                  ? "Guest Preferences"
                  : "Complete"}
          </button>
        )
    ...
    <div className={styles.wizard}>
      <div className={styles.wizardHeader}>
        ...
        {onboardingData && onboardingData.completedSteps > 0 && (
          <StepNavigation currentStep={currentStep} completedSteps={onboardingData.completedSteps} onGoToStep={handleGoToStep} />
        )}
      </div>

      <div className={styles.wizardContent}>
        {match(currentStep)
          .with("personal-info", () => <PersonalInfoStep onNext={handleNextStep} initialData={onboardingData?.personalInfo} />)
          ...
          .with("onboarding-complete", () => <OnboardingCompleteStep />)
          .exhaustive()}
      </div>
    </div>
  )
